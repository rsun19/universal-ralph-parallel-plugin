...
phase_plan() {
  ...
  local plan_prompt="${STATE_DIR}/prompts/.manager-plan-prompt.md"
  mkdir -p "${STATE_DIR}/prompts"
  local plan_template="${RALPH_ROOT}/state/templates/prompt-plan.md"

  local prompt_content
  prompt_content=$(cat "$PROMPT_FILE")

  if [[ -f "$plan_template" ]]; then
    render_template "$plan_template" \
      TARGET_REPO "$TARGET_REPO" \
      PROMPT_FILE "$prompt_content" > "$plan_prompt"
  else
    ralph_die "Missing template: $plan_template (run 'ralph templates' to see all templates)"
  fi

  local plan_output="${STATE_DIR}/plan-output.json"
  ...
  echo "$tasks_json" > "${STATE_DIR}/plan.json"

  # Write fix_plan.md in session state (not the target repo)
  {
  ...
  } > "${STATE_DIR}/fix_plan.md" 2>/dev/null || true
}
...
manager_loop() {
  ...
  while true; do
    ...
    if all_tasks_done; then
      ...
      } > "${STATE_DIR}/completion-report.md"
